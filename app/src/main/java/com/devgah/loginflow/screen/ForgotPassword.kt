...
fun Checking() {
    ...
    when (currentScreen) {
        "page" -> Page(
            onLoginSucess = {currentScreen = "login" }, onForgotPassword = {currentScreen = "forgot"}
        )
    }
}

@Composable
fun ForgotPassword(onBack: () -> Unit, onPage: () -> Unit) {
    ...
    Column(
        ...
        Button(
            onClick = {
                if (email.isBlank()) {
                    Toast.makeText(context, "É preciso preencher o campo!", Toast.LENGTH_SHORT).show()
                } else {
                    if (email.length <= 3) {
                        Toast.makeText(context, "É preciso informar no mínimo 4 caracteres", Toast.LENGTH_SHORT).show()
                    } else {
