...
fun SignUp(onSignUp: () -> Unit, onPage: () -> Unit) {
    ...
    Column(
        ...
        Text(
            text = "Cadastre-se para iniciar a sua jornada",
            color = Color.White,
            fontSize = 13.5.sp,
        ...
        Button(
            onClick = {
                if (textEmail.isBlank() || textPassword.isBlank() || textUsername.isBlank()) {
                    Toast.makeText(context, "É preciso preencher os campos!", Toast.LENGTH_SHORT).show()
                } else {
                    if (textEmail.length < 3 || textPassword.length <= 3 || textUsername.length <= 3) {
                        Toast.makeText(context, "É preciso inserir no mínimo 4 caracteres!", Toast.LENGTH_SHORT).show()
                    } else {
